...
let largeShirts = [];
    largeShirts = runners.filter(function(shirt) {
...
let ticketPriceTotal = [];
   ticketPriceTotal = runners.reduce(function(num, donations) {
        return num + donations.donation;
    },0);
...
//get emails
let iMail = [];
    runners.forEach(function (mail) {
        iMail.push(mail.email);
        return iMail;
    });
...
//get id
let indenity = [];
    runners.forEach(function (num) {
        indenity.push(num.id);
        return indenity;
    });

console.log(indenity);

// Problem 3
//get company name
let cName = [];
    runners.forEach(function (compname) {
        cName.push(compname.company_name);
        return cName;
    });
